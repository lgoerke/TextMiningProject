...
def lstm(print_number):
	...
	logname = 'log_l_imp_' + print_number +'.txt'
	f = open(logname, 'w')
	sys.stdout = f
	sys.stderr = f
	...
	filepath= 'weights_l_imp_' + str(print_number) + '/weights-improvement-e{epoch:02d}-l{loss:.4f}-a{acc:.4f}.hdf5'
	mkdir_p(os.path.dirname(filepath))
	checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1)
	callbacks_list = [checkpoint]

	model.fit(reviews_train, prices_train, nb_epoch=20, batch_size=64, callbacks=callbacks_list)
	# Final evaluation of the model
	predicted = model.predict(reviews_test, verbose=0)
	scores = model.evaluate(reviews_test, prices_test, verbose=0)
	print("Accuracy: %.2f%%" % (scores[1]*100))
	pickle.dump(predicted,open('data/predicted_classes_l_adagrad.pkl','wb'))
	print(predicted)
...
def printUsage():
	print("Usage: 09_lstm.py {Number of run}")
